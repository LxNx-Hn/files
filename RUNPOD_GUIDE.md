...
현재 기준의 최종 실험은 `data -> single10 -> single10_balanced -> phase1(CNN) -> phase2(RES)` 흐름입니다.  
중간에 끊겨도 1차 결과를 보존하기 위해 두 단계로 나눠서 실행합니다.
...
zip -r tmdb_multimodal_runpod.zip \
  run_all.sh launcher.py experiments_single10_final_phase1_cnn.json experiments_single10_final_phase2_res.json \
...
python3 launcher.py --config experiments_single10_final_phase2_res.json --dry_run
python3 launcher.py --config experiments_single10_final_phase2_res.json
tail -f results_single10_final_phase1_cnn/experiment.log
watch -n 5 nvidia-smi
```

## 중단 후 재시작

런처는 이미 각 결과 폴더의 `runs/<experiment>/summary.json` 이 있는 실험은 자동으로 건너뜁니다. 그래서 Pod가 끊기거나 SSH가 끊겨도 같은 명령을 다시 치면 됩니다.
...
```bash
python3 launcher.py --config experiments_single10_final_phase1_cnn.json --aggregate_only
python3 launcher.py --config experiments_single10_final_phase2_res.json --aggregate_only
```
